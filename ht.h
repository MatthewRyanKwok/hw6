...
HashTable<K, V, Prober, Hash, KEqual>::HashTable(
    ...
    : hash_(hash), kequal_(kequal), prober_(prober), totalProbes_(0), mIndex_(0), size_(0), resizeAlpha_(resizeAlpha)
{
...
{
    double loadFactor = static_cast<double>(size_ + 1) / static_cast<double>(CAPACITIES[mIndex_]);
    if (!resizing_ && loadFactor >= resizeAlpha_) {
        resize();
    }
    ...
    else if (table_[index]->item.second != p.second) {
        table_[index]->item.second = p.second; // Update value
    }
}
...
void HashTable<K, V, Prober, Hash, KEqual>::remove(const KeyType& key)
{
    ...
    if (index == npos || table_[index] == nullptr || table_[index]->deleted) {
        return;  // No item found to remove
    }

    // Mark the item as deleted (tombstone) instead of actually removing it
    table_[index]->deleted = true;
    --size_;  // Decrement size to reflect removal
...
{ 
   // std::cout << "Resizing: old size = " << table_.size() << ", new capacity = " << CAPACITIES[mIndex_] << std::endl;

    mIndex_++;
    static const size_t CAPACITY_COUNT = sizeof(CAPACITIES) / sizeof(CAPACITIES[0]);
    if (mIndex_ >= CAPACITY_COUNT) {
        throw std::logic_error("HashTable capacity limit reached");
    }
    ...
    for (size_t i = 0; i < oldTable.size(); ++i) {
        if (oldTable[i] != nullptr && !oldTable[i]->deleted) {
            insert(oldTable[i]->item);  // Rehash live items
        }
        delete oldTable[i];  // Always delete the old pointer
    }
    resizing_ = false;
}
